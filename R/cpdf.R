...
#' @export
cpdf <- function(object, newdata, log = FALSE, cores = 1, ...) {
  ...
  u <- get_pits(olddata = object$model_frame,
                ...
                uscale = uscale,
                cores = cores)
  psobs_old <-  list(psobs = list(direct = array(u[, 1], dim = c(1, 1, nrow(u))),
  ...
  cond_dist <- function(new_var, pair_copulas, psobs_old) {
    ...
    return(list(psobs = psobs, dens = dens))
  ...
  for (var in object$order) {
    psobs_old <- cond_dist(new_var = u[, var],
                           ...
                           psobs_old = psobs_old$psobs)
    dens <- dens * psobs_old$dens
  }

  dens <- dens * unlist(get_density(olddata = object$model_frame,
                                    newdata = mf,
